...
public class NotParam extends NormalParam implements Param {
	...
	public String buildOperate(String column) {
		if (value == null) {
			return GenericDaoUtils.connect(column, " is not null");
	...
	private String buildCollectionOperate(String column) {
		...
		if (coll.size() == 1) {
			...
			return GenericDaoUtils.connect(column, " not in ", buildInOperate(coll.size()));
		}
	}
	...
	private String buildArrayOperate(String column) {
		...
		if (array.length == 1) {
			...
			return GenericDaoUtils.connect(column, " not in ", buildInOperate(array.length));
		}
	}
}
