...
public class PlanDaoImpl extends GenericDao<Integer, Plan> implements PlanDao {
	...
	public List<Plan> pageByUserId(int userId, int offset, int rows) {
		return page(Matches.one(UserObject.Columns.userId, userId), null, offset, rows);
	}

	public List<Plan> pageSimpleByUserId(int userId, int offset, int rows) {
		return findBySQL(Plan.class, "select * from plan where userid=? limit ?, ?", Utils.toObjectList(userId, offset, rows));
	}

	public List<String> pagePlanNameByUserId(int userId, int offset, int rows) {
		return pageOneColumn(String.class, Matches.one(UserObject.Columns.userId, userId), Plan.Columns.planName, offset, rows);
	}
